...
export default function SiteHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#06101f]/90 text-white backdrop-blur-2xl">
    <div className="container flex h-[78px] items-center justify-between gap-4">
      <a href="/" onClick={() => setOpen(false)} className="flex shrink-0 items-center rounded-xl bg-white px-2.5 py-1.5 shadow-[0_8px_28px_rgba(0,0,0,.16)]" aria-label="Mega Stride Technologies home">
        <img src="/mega-stride-logo.webp" alt="Mega Stride Technologies Ltd" className="h-11 w-[176px] object-contain sm:h-12 sm:w-[210px]" />
      </a>
      <nav className="hidden items-center gap-6 lg:flex" aria-label="Primary navigation">{links.map(([label, href]) => <a key={label} href={href} className="nav-link whitespace-nowrap">{label}</a>)}</nav>
      <div className="hidden items-center gap-2 md:flex"><a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="Chat with Mega Stride Technologies on WhatsApp" className="flex items-center gap-2 rounded-full border border-[#25D366]/30 bg-[#25D366]/10 px-3 py-2 text-xs font-black text-[#b7ffd2] transition hover:border-[#25D366]/60 hover:bg-[#25D366]/20"><WhatsAppIcon /> WhatsApp</a><a href={facebookUrl} target="_blank" rel="noreferrer" aria-label="Mega Stride Technologies on Facebook" className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-slate-300 transition hover:-translate-y-0.5 hover:border-blue-400/50 hover:bg-blue-500/15 hover:text-blue-300"><FacebookIcon /></a><a href="/contact" className="rounded-full border border-white/10 px-4 py-2.5 text-sm font-bold text-slate-200 transition hover:bg-white/5">Contact</a><a href="/contact" className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-5 py-2.5 text-sm font-bold text-cyan-100 transition hover:border-cyan-200/60 hover:bg-cyan-300/20">Start a conversation <span className="ml-2">↗</span></a></div>
