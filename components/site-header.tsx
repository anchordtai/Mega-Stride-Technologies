...
export default function SiteHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#06101f]/78 text-white backdrop-blur-2xl">
    ...
    {open && <nav className="border-t border-white/10 bg-[#06101f]/95 px-5 py-5 md:hidden" aria-label="Mobile navigation"><div className="container flex flex-col gap-1">{links.map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)} className="rounded-xl px-3 py-3 text-slate-200 hover:bg-white/5">{label}</a>)}<a href={whatsappUrl} target="_blank" rel="noreferrer" onClick={() => setOpen(false)} className="flex items-center gap-3 rounded-xl bg-[#25D366]/10 px-3 py-3 font-bold text-[#b7ffd2] hover:bg-[#25D366]/20"><WhatsAppIcon /> Chat on WhatsApp</a><a href={facebookUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-xl px-3 py-3 text-slate-200 hover:bg-white/5"><FacebookIcon /> Facebook</a><a href="/contact" onClick={() => setOpen(false)} className="mt-2 rounded-xl bg-cyan-300 px-4 py-3 text-center font-bold text-[#06101f]">Start a conversation</a></div></nav>}
  </header>;
}
